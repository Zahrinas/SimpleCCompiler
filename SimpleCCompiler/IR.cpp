...
IR_funct IRdata_LLVM::parseFunct(AST* ast) {
	...
	for (AST* ptr = ast->son(3); ptr; ptr = ptr->next) {
		++ret.instCnt;
		ret.args.push_back(ptr->son(0)->data.toLLVM_type());
		ret.regs.push_back("%" + std::to_string(ret.instCnt));
	}
...
Expression IRdata_LLVM::parseSequence(IR_funct& fun, AST* ast) {
	...
	case dataType::constant: {
		++fun.instCnt;
		fun.body.push_back("%" + std::to_string(fun.instCnt) + " = alloca " + ast->data.toLLVM_type());
		fun.body.push_back("store " + ast->data.toLLVM_type() + " " + ast->data.toStringExpr() + " , " 
			+ ast->data.toLLVM_type() + "* %" + std::to_string(fun.instCnt));
		++fun.instCnt;
		fun.body.push_back("%" + std::to_string(fun.instCnt)
			+ " = load " + ast->data.toLLVM_type() + " , " + ast->data.toLLVM_type() + "* %" + std::to_string(fun.instCnt - 1));
		return Expression(ast->data.toLLVM_type(), "%" + std::to_string(fun.instCnt));
	}
	case dataType::string: {
		++fun.instCnt;
		fun.body.push_back("%" + std::to_string(fun.instCnt) + " = getelementptr inbounds "
			+ ast->data.toLLVM_type() + " , " + ast->data.toLLVM_type() + "* "
			+ getVarId(fun, ast->data) + " , i64 0, i64 0");
	...
	case dataType::call_inst: {
		if (ast->son(0)->data.value.getDataString() == "scanf") {
			...
			++fun.instCnt;
			str = "%" + std::to_string(fun.instCnt) + " = call i64 (i8*, ...) @scanf (" + a.type + " " + a.value + str + ")";
			...
			Expression a = parseSequence(fun, ast->son(1));
			++fun.instCnt;
			str = "%" + std::to_string(fun.instCnt) + " = call i64 (i8*, ...) @printf (" + a.type + " " + a.value + str + ")";
			fun.body.push_back(str);
			return Expression("i64" , "%" + std::to_string(fun.instCnt));
		}
			...
			Expression a = parseSequence(fun, ast->son(1));
			++fun.instCnt;
			std::string str = "%" + std::to_string(fun.instCnt) + " = call i8* (i64) @malloc ("
				+ a.type + " "+ a.value + ")";
			fun.body.push_back(str);
			return Expression("i8*" , "%" + std::to_string(fun.instCnt));
		}
...
void IRdata_LLVM::printIR(std::string filename) {
	...
	for (int i = 0; i < functs.size(); ++i) {
		...
		for (int j = 0; j < functs[i].args.size(); ++j) {
			if (j) fout << " , ";
			fout << functs[i].args[j] << ' ' << functs[i].regs[j];
		}
		fout<< ") {" << std::endl;
	...
	fout << "declare i64 @scanf(i8*, ...)" << std::endl;
	fout << "declare i64 @printf(i8*, ...)" << std::endl;
	fout << "declare i8* @malloc(i64)" << std::endl;
	fout << "declare i8* @strchr(i8*, i64)" << std::endl;
	fout << "declare i64 @sscanf(i8*, i8*, ...)" << std::endl;
	fout << "declare i64 @strcmp(i8*, i8*)" << std::endl;
	fout << "declare void @memset(i8*, i8, i64)" << std::endl;
	fout << "declare i64 @gets(...)" << std::endl;
}
